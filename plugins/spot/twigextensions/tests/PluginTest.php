<?php namespace Spot\TwigExtensions\Tests;
...
use Spot\TwigExtensions\Classes\TimeDiffTranslator;
...
class PluginTest extends PluginTestCase
{
    ...
    public function testMailtoFilter()
    {
        ...
        $template = "{{ 'spot.cz@gmail.com' | mailto }}";
        $twigTemplate = $twig->createTemplate($template);
        $this->assertNotContains('spot.cz@gmail.com', $twigTemplate->render([]));
        $this->assertContains('mailto:', $twigTemplate->render([]));

        // mailto(false, false) eg. without link and unprotected
        $template = "{{ 'spot.cz@gmail.com' | mailto(false, false) }}";
        $twigTemplate = $twig->createTemplate($template);
        $this->assertContains('spot.cz@gmail.com', $twigTemplate->render([]));
        $this->assertNotContains('mailto:', $twigTemplate->render([]));

        // mailto(true, false) eg. with link but unprotected
        $template = "{{ 'spot.cz@gmail.com' | mailto(true, false) }}";
        $twigTemplate = $twig->createTemplate($template);
        $this->assertContains('spot.cz@gmail.com', $twigTemplate->render([]));
        $this->assertContains('mailto', $twigTemplate->render([]));

        // mailto(false, true) eg. without link and protected
        $template = "{{ 'spot.cz@gmail.com' | mailto(false, true) }}";
        $twigTemplate = $twig->createTemplate($template);
        $this->assertNotContains('spot.cz@gmail.com', $twigTemplate->render([]));
        $this->assertNotContains('mailto', $twigTemplate->render([]));

        // mailto(true, true, 'Let me know') eg. with link, protected and with non-crypted text
        $template = "{{ 'spot.cz@gmail.com' | mailto(false, true, 'Let me know') }}";
        $twigTemplate = $twig->createTemplate($template);
        $this->assertNotContains('spot.cz@gmail.com', $twigTemplate->render([]));
        $this->assertNotContains('mailto', $twigTemplate->render([]));
